    ...
    def install(self):
        ...
        for layer_idx in self.target_layers:
            ...
            for name, proj in targets:
                ...
                # Project out the harmful direction from each output column
                # W_new = W - α * (W @ d) d^T  where d ∈ ℝ^d_in
                # For o_proj: d_in = head_dim * n_heads, d_out = d_model
                # The direction lives in d_out space here
                
                # Actually abliterate from rows (output directions)
                # W rows = output vectors, we remove the harmful component
                d_out = direction[:W.shape[0]].to(W.device)  # match dim
                if d_out.shape[0] != W.shape[0]:
                    log.warning(
                        f"Layer {layer_idx} {name}: direction dim mismatch "
                        f"({direction.shape[0]} vs {W.shape[0]}), skipping."
                    )
                    continue
                
                # proj_out[i] = W[i] - α*(W[i]·d)*d
                proj_coeff = (W @ d_out).unsqueeze(1)  # (d_out, 1)
                W_new = W - self.steering_intensity * proj_coeff * d_out.unsqueeze(0)
