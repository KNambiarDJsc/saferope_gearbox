...
class SafeRoPEGearbox:
    ...
    def __init__(
        ...
        # Gemma-2 stores head_dim explicitly in config (256),

        # which differs from hidden_size // num_heads (2304 // 8 = 288).

        # Always prefer the explicit value when present.

        model_cfg = model.config

        if hasattr(model_cfg, 'head_dim'):
        ...
        for layer_idx in target_layers:
            ...
            # Probe weight shape: (1, hidden_size).

            # CommutativeProjector operates at head_dim granularity.

            # Truncate to head_dim and renormalise — this takes the

            # harm direction projected onto the first head's subspace.

            # All heads share the same R matrix (uniform steering).

            raw_w = probe_weights[layer_idx]  # (1, hidden_size)

            w_head = raw_w[:, :self.head_dim].clone()

            w_head = w_head / w_head.norm().clamp(min=1e-8)

            extractor.fit(w_head)
