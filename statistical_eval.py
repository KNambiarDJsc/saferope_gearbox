...
class StatisticalEvaluator:
    ...
    def run_commutativity_test(self, n_angles: int = 100) -> Dict:
        ...
        head_dim = model_cfg.head_dim if hasattr(model_cfg, "head_dim") else model_cfg.hidden_size // model_cfg.num_attention_heads
        ...
        for layer_idx, probe_W in self.probe_weights.items():
            ...
            # Probe weight is (1, hidden_size). Truncate to head_dim for

            # the per-head projector — we take the first head_dim dims as

            # a representative slice of the harm direction.

            probe_W_head = probe_W[:, :head_dim].clone()

            # Re-normalize after truncation

            probe_W_head = probe_W_head / probe_W_head.norm().clamp(min=1e-8)

            extractor.fit(probe_W_head)
